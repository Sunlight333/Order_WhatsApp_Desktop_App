...
async function createOrder(userId, input) {
    ...
    const orderId = await prisma.$transaction(async (tx) => {
        ...
        const findOrCreateSupplierInTx = async (name) => {
            const trimmedName = name.trim();
            const allSuppliers = await tx.supplier.findMany();
            let supplier = allSuppliers.find((s) => s.name.toLowerCase() === trimmedName.toLowerCase());
            if (!supplier) {
        ...
        const findOrCreateProductInTx = async (supplierId, reference) => {
            const trimmedRef = reference.trim();
            const products = await tx.product.findMany({
                where: { supplierId },
            });
            let product = products.find((p) => p.reference.toLowerCase() === trimmedRef.toLowerCase());
            if (!product) {
 ...
 * Update order status
 */
async function updateOrderStatus(orderId, userId, status, notificationMethod) {
    const validStatuses = ['PENDING', 'RECEIVED', 'NOTIFIED_CALL', 'NOTIFIED_WHATSAPP'];
    if (!validStatuses.includes(status)) {
        throw (0, error_util_1.createError)('INVALID_STATUS', 'Invalid order status', 400);
    }
    // Get current order
    ...
    if (!currentOrder) {
        throw (0, error_util_1.createError)('ORDER_NOT_FOUND', 'Order not found', 404);
    }
 ...
 */
async function updateOrder(orderId, userId, input) {
